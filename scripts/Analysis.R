install.packages(c("tidyverse", "broom", "ggpubr"))

data_file <- "data/gpa.csv"
out_plot_dir <- "plots"
...
library(ggpubr)
library(readr)
data_path <- "C:/Users/gm25aav/Desktop/7COM1079-Group-A-68/data/gpa.csv"
df <- read_csv(data_path, show_col_types = FALSE)
head(df)
df <- df %>% mutate(gpa = as.numeric(gpa),studyweek = as.numeric(studyweek),sleepnight = as.numeric(sleepnight),out = as.numeric(out),gender = as.factor(gender)) %>% filter(!is.na(gpa) & !is.na(studyweek))
summary_stats <- tibble(
  n = nrow(df),
  mean_gpa = mean(df$gpa),
  sd_gpa = sd(df$gpa),
  mean_studyweek = mean(df$studyweek),
  sd_studyweek = sd(df$studyweek),
  min_gpa = min(df$gpa),
  max_gpa = max(df$gpa),
  min_studyweek = min(df$studyweek),
  max_studyweek = max(df$studyweek)
)
...
png(file.path(out_plot_dir, "qqplot_residuals.png"), width = 800, height = 600, res = 100)
qqnorm(lm_model$residuals); qqline(lm_model$residuals, col = "red")
dev.off()
cat("Sample size (n):", summary_stats$n, "\n\n")
cat("Pearson correlation: r =", round(pearson$estimate, 4), ", p =", format.pval(pearson$p.value, digits = 6), "\n\n")
